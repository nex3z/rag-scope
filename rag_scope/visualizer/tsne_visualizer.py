...
class TsneVisualizer(ModelBasedVisualizer):
    ...
    def transform(self, embeddings: NDArray) -> NDArray:
        model = TSNE(n_components=self.n_components, random_state=self.random_state)
...
def main():
    ...
    viz = TsneVisualizer(vector_store.get_stored_documents(), n_components=2, random_state=42)
